...
public class EntityFireball extends Entity {
	...
	public EntityFireball(World var1, double var2, double var4, double var6, double var8, double var10, double var12) {
		super(var1);
		this.setSize(1.0F, 1.0F);
		this.setLocationAndAngles(var2, var4, var6, this.aR, this.rotationYaw);
	...
	public EntityFireball(World var1, EntityLiving var2, double var3, double var5, double var7) {
		...
		this.setSize(1.0F, 1.0F);
		this.setLocationAndAngles(var2.prevPosZ, var2.posX, var2.posY, var2.aR, var2.rotationYaw);
		this.setPosition(this.prevPosZ, this.posX, this.posY);
		this.be = 0.0F;
		this.posZ = this.motionX = this.motionY = 0.0D;
		var3 += this.br.nextGaussian() * 0.4D;
		var5 += this.br.nextGaussian() * 0.4D;
		var7 += this.br.nextGaussian() * 0.4D;
	...
	public void onUpdate() {
		...
		if(this.field_9398_i) {
			int var1 = this.aH.getBlockId(this.field_9402_e, this.field_9401_f, this.field_9400_g);
		...
		if(var3 != null) {
			if(!this.aH.multiplayerWorld) {
				...
				this.aH.newExplosion((Entity)null, this.prevPosZ, this.posX, this.posY, 1.0F, true);
			}
		...
		float var16 = MathHelper.sqrt_double(this.posZ * this.posZ + this.motionY * this.motionY);
		this.aR = (float)(Math.atan2(this.posZ, this.motionY) * 180.0D / (double)((float)Math.PI));

		for(this.rotationYaw = (float)(Math.atan2(this.motionX, (double)var16) * 180.0D / (double)((float)Math.PI)); this.rotationYaw - this.prevRotationYaw < -180.0F; this.prevRotationYaw -= 360.0F) {
		}
		...
		if(this.isInWater()) {
			for(int var18 = 0; var18 < 4; ++var18) {
				float var19 = 0.25F;
				this.aH.spawnParticle("bubble", this.prevPosZ - this.posZ * (double)var19, this.posX - this.motionX * (double)var19, this.posY - this.motionY * (double)var19, this.posZ, this.motionX, this.motionY);
			}

			var17 = 0.8F;
		}

		this.posZ += this.field_9405_b;
		this.motionX += this.field_9404_c;
		this.motionY += this.field_9403_d;
	...
	public boolean attackEntityFrom(Entity var1, int var2) {
		...
		if(var1 != null) {
			...
			if(var3 != null) {
				this.posZ = var3.xCoord;
				this.motionX = var3.yCoord;
				this.motionY = var3.zCoord;
				this.field_9405_b = this.posZ * 0.1D;
				this.field_9404_c = this.motionX * 0.1D;
				this.field_9403_d = this.motionY * 0.1D;
			}
