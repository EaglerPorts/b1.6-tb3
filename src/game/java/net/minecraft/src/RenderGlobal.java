...
public class RenderGlobal implements IWorldAccess {
	...
	public void loadRenderers() {
			...
			for (int var5 = 0; var5 < this.renderChunksTall; ++var5) {
				for (int var6 = 0; var6 < this.renderChunksDeep; ++var6) {
					this.worldRenderers[(var6 * this.renderChunksTall + var5) * this.renderChunksWide + var4] = new WorldRenderer(
							this.worldObj, this.tileEntities, var4 * 16, var5 * 16, var6 * 16, 16, this.glRenderListBase + var2);
		...
		if (this.worldObj != null) {
			...
			if (var7 != null) {
				this.markRenderersForNewPosition(MathHelper.floor_double(var7.prevPosZ), MathHelper.floor_double(var7.posX),
						MathHelper.floor_double(var7.posY));
	...
	public void renderEntities(Vec3D var1, ICamera var2, float var3) {
		if (this.renderEntitiesStartupCounter > 0) {
			...
			TileEntityRenderer.instance.cacheActiveRenderInfo(this.worldObj, this.renderEngine, this.mc.fontRenderer,
					this.mc.renderViewEntity, var3);
			RenderManager.instance.cacheActiveRenderInfo(this.worldObj, this.renderEngine, this.mc.fontRenderer,
					this.mc.renderViewEntity, this.mc.gameSettings, var3);
			this.countEntitiesTotal = 0;
			this.countEntitiesRendered = 0;
			this.countEntitiesHidden = 0;
			EntityLiving var4 = this.mc.renderViewEntity;
			RenderManager.renderPosX = var4.bk + (var4.prevPosZ - var4.bk) * (double) var3;
			RenderManager.renderPosY = var4.lastTickPosX + (var4.posX - var4.lastTickPosX) * (double) var3;
			RenderManager.renderPosZ = var4.lastTickPosY + (var4.posY - var4.lastTickPosY) * (double) var3;
			TileEntityRenderer.staticPlayerX = var4.bk + (var4.prevPosZ - var4.bk) * (double) var3;
			TileEntityRenderer.staticPlayerY = var4.lastTickPosX + (var4.posX - var4.lastTickPosX) * (double) var3;
			TileEntityRenderer.staticPlayerZ = var4.lastTickPosY + (var4.posY - var4.lastTickPosY) * (double) var3;
			...
			for (var6 = 0; var6 < var5.size(); ++var6) {
				...
				if (var7.isInRangeToRenderVec3D(var1) && (var7.bK || var2.isBoundingBoxInFrustum(var7.aV))
						&& (var7 != this.mc.renderViewEntity || this.mc.gameSettings.thirdPersonView
								|| this.mc.renderViewEntity.isPlayerSleeping())) {
					int var8 = MathHelper.floor_double(var7.posX);
					if (var8 < 0) {
						var8 = 0;
					}
					...
					if (this.worldObj.blockExists(MathHelper.floor_double(var7.prevPosZ), var8,
							MathHelper.floor_double(var7.posY))) {
						++this.countEntitiesRendered;
						RenderManager.instance.renderEntity(var7, var3);
					}
	...
	public int sortAndRender(EntityLiving var1, int var2, double var3) {
		...
		double var33 = var1.bk + (var1.prevPosZ - var1.bk) * var3;
		double var7 = var1.lastTickPosX + (var1.posX - var1.lastTickPosX) * var3;
		double var9 = var1.lastTickPosY + (var1.posY - var1.lastTickPosY) * var3;
		double var11 = var1.prevPosZ - this.prevSortX;
		double var13 = var1.posX - this.prevSortY;
		double var15 = var1.posY - this.prevSortZ;
		if (var11 * var11 + var13 * var13 + var15 * var15 > 16.0D) {
			this.prevSortX = var1.prevPosZ;
			this.prevSortY = var1.posX;
			this.prevSortZ = var1.posY;
			this.markRenderersForNewPosition(MathHelper.floor_double(var1.prevPosZ), MathHelper.floor_double(var1.posX),
	...
	private int renderSortedRenderers(int var1, int var2, int var3, double var4) {
		...
		double var20 = var19.bk + (var19.prevPosZ - var19.bk) * var4;
		double var10 = var19.lastTickPosX + (var19.posX - var19.lastTickPosX) * var4;
		double var12 = var19.lastTickPosY + (var19.posY - var19.lastTickPosY) * var4;
		int var14 = 0;
	...
	public void renderClouds(float var1) {
		if (!this.mc.theWorld.worldProvider.isNether) {
			if (this.mc.gameSettings.fancyGraphics) {
				...
				float var2 = (float) (this.mc.renderViewEntity.lastTickPosX
						+ (this.mc.renderViewEntity.posX - this.mc.renderViewEntity.lastTickPosX) * (double) var1);
				byte var3 = 32;
				int var4 = 256 / var3;
				...
				var10 = 0.5F / 1024.0F;
				double var22 = this.mc.renderViewEntity.aI
						+ (this.mc.renderViewEntity.prevPosZ - this.mc.renderViewEntity.aI) * (double) var1
						+ (double) (((float) this.cloudOffsetX + var1) * 0.03F);
				double var13 = this.mc.renderViewEntity.prevPosY
						+ (this.mc.renderViewEntity.posY - this.mc.renderViewEntity.prevPosY) * (double) var1;
	...
	public void renderCloudsFancy(float var1) {
		...
		float var4 = 12.0F;
		float var5 = 4.0F;
		double var6 = (this.mc.renderViewEntity.aI
				+ (this.mc.renderViewEntity.prevPosZ - this.mc.renderViewEntity.aI) * (double) var1
				+ (double) (((float) this.cloudOffsetX + var1) * 0.03F)) / (double) var4;
		double var8 = (this.mc.renderViewEntity.prevPosY
				+ (this.mc.renderViewEntity.posY - this.mc.renderViewEntity.prevPosY) * (double) var1) / (double) var4
				+ (double) 0.33F;
	...
	public boolean updateRenderers(EntityLiving var1, boolean var2) {
		...
		if (var3) {
			...
			int var12;
			int var13;
			label169: for (var10 = 0; var10 < var8; ++var10) {
	...
	public void drawBlockBreaking(EntityPlayer var1, MovingObjectPosition var2, int var3, ItemStack var4, float var5) {
		...
		if (var3 == 0) {
			if (this.damagePartialTime > 0.0F) {
				...
				double var10 = var1.bk + (var1.prevPosZ - var1.bk) * (double) var5;
				double var12 = var1.lastTickPosX + (var1.posX - var1.lastTickPosX) * (double) var5;
				double var14 = var1.lastTickPosY + (var1.posY - var1.lastTickPosY) * (double) var5;
				if (var9 == null) {
					var9 = Block.stone;
				}
				...
				var6.disableColor();
				this.globalRenderBlocks.renderBlockUsingTexture(var9, var2.blockX, var2.blockY, var2.blockZ,
	...
	public void drawSelectionBox(EntityPlayer var1, MovingObjectPosition var2, int var3, ItemStack var4, float var5) {
		if (var3 == 0 && var2.typeOfHit == EnumMovingObjectType.TILE) {
			...
			if (var7 > 0) {
				...
				double var8 = var1.bk + (var1.prevPosZ - var1.bk) * (double) var5;
	...
	public void clipRenderersByFrustrum(ICamera var1, float var2) {
		for (int var3 = 0; var3 < this.worldRenderers.length; ++var3) {
			if (!this.worldRenderers[var3].skipAllRenderPasses()
					&& (!this.worldRenderers[var3].isInFrustum || (var3 + this.frustrumCheckOffset & 15) == 0)) {
	...
	public void spawnParticle(String var1, double var2, double var4, double var6, double var8, double var10,
		...
		if (this.mc != null && this.mc.renderViewEntity != null && this.mc.effectRenderer != null) {
			double var14 = this.mc.renderViewEntity.prevPosZ - var2;
			double var16 = this.mc.renderViewEntity.posX - var4;
			double var18 = this.mc.renderViewEntity.posY - var6;
			double var20 = 16.0D;
			if (var14 * var14 + var16 * var16 + var18 * var18 <= var20 * var20) {
				if (var1.equals("bubble")) {
					...
					this.mc.effectRenderer
							.addEffect(new EntitySmokeFX(this.worldObj, var2, var4, var6, var8, var10, var12, 2.5F));
	...
	public void releaseEntitySkin(Entity var1) {
		if (var1.bz != null) {
			this.renderEngine.releaseImageData(var1.bz);
		}
	...
	public void func_28136_a(EntityPlayer var1, int var2, int var3, int var4, int var5, int var6) {
		...
		switch (var2) {
			...
			case 1003:
				if (Math.random() < 0.5D) {
					this.worldObj.playSoundEffect((double) var3 + 0.5D, (double) var4 + 0.5D, (double) var5 + 0.5D,
							"random.door_open", 1.0F, this.worldObj.rand.nextFloat() * 0.1F + 0.9F);
				} else {
					this.worldObj.playSoundEffect((double) var3 + 0.5D, (double) var4 + 0.5D, (double) var5 + 0.5D,
							"random.door_close", 1.0F, this.worldObj.rand.nextFloat() * 0.1F + 0.9F);
				}
				break;
			case 1004:
				this.worldObj.playSoundEffect((double) ((float) var3 + 0.5F), (double) ((float) var4 + 0.5F),
						(double) ((float) var5 + 0.5F), "random.fizz", 0.5F, 2.6F + (var7.nextFloat() - var7.nextFloat()) * 0.8F);
				break;
			case 1005:
			...
			case 2001:
				...
				if (var16 > 0) {
					...
					this.mc.sndManager.playSound(var17.stepSound.stepSoundDir(), (float) var3 + 0.5F, (float) var4 + 0.5F,
							(float) var5 + 0.5F, (var17.stepSound.getVolume() + 1.0F) / 2.0F, var17.stepSound.getPitch() * 0.8F);
				}
