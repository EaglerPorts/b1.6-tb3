...
public abstract class Entity {
	...
	public void setPosition(double var1, double var3, double var5) {
		...
		this.aV.setBounds(var1 - (double) var7, var3 - (double) this.be + (double) this.bn, var5 - (double) var7,
				var1 + (double) var7, var3 - (double) this.be + (double) this.bn + (double) var8, var5 + (double) var7);
	}
	...
	public void onEntityUpdate() {
		...
		if (this.handleWaterMovement()) {
			if (!this.bw && !this.isFirstUpdate) {
				float var1 = MathHelper.sqrt_double(this.posZ * this.posZ * (double) 0.2F + this.motionX * this.motionX
						+ this.motionY * this.motionY * (double) 0.2F) * 0.2F;
				if (var1 > 1.0F) {
					var1 = 1.0F;
				}

				this.aH.playSoundAtEntity(this, "random.splash", var1,
						1.0F + (this.br.nextFloat() - this.br.nextFloat()) * 0.4F);
				...
				for (var3 = 0; (float) var3 < 1.0F + this.yOffset * 20.0F; ++var3) {
					...
					this.aH.spawnParticle("bubble", this.prevPosZ + (double) var4, (double) (var2 + 1.0F),
							this.posY + (double) var5, this.posZ, this.motionX - (double) (this.br.nextFloat() * 0.2F), this.motionY);
				}

				for (var3 = 0; (float) var3 < 1.0F + this.yOffset * 20.0F; ++var3) {
					var4 = (this.br.nextFloat() * 2.0F - 1.0F) * this.yOffset;
					var5 = (this.br.nextFloat() * 2.0F - 1.0F) * this.yOffset;
					this.aH.spawnParticle("splash", this.prevPosZ + (double) var4, (double) (var2 + 1.0F),
							this.posY + (double) var5, this.posZ, this.motionX, this.motionY);
				}
			}
		...
		if (this.bp) {
			...
			if (var18) {
				...
				for (var19 = 0.05D; var1 != 0.0D
						&& this.aH.getCollidingBoundingBoxes(this, this.aV.getOffsetBoundingBox(var1, -1.0D, 0.0D))
								.size() == 0; var11 = var1) {
				...
				for (; var5 != 0.0D && this.aH.getCollidingBoundingBoxes(this, this.aV.getOffsetBoundingBox(0.0D, -1.0D, var5))
						.size() == 0; var15 = var5) {
			...
			if (this.canTriggerWalking() && !var18 && this.riddenByEntity == null) {
				this.prevDistanceWalkedModified = (float) ((double) this.prevDistanceWalkedModified
	...
	public boolean isWet() {
		return this.bw || this.aH.canBlockBeRainedOn(MathHelper.floor_double(this.prevPosZ),
	...
	public boolean handleWaterMovement() {
		return this.aH.handleMaterialAcceleration(
				this.aV.expand(0.0D, (double) -0.4F, 0.0D).func_28195_e(0.001D, 0.001D, 0.001D), Material.water, this);
	}
	...
	public float getEntityBrightness(float var1) {
		...
		int var6 = MathHelper.floor_double(this.posY);
		return this.aH.checkChunksExist(MathHelper.floor_double(this.aV.minX), MathHelper.floor_double(this.aV.minY),
				MathHelper.floor_double(this.aV.minZ), MathHelper.floor_double(this.aV.maxX),
				MathHelper.floor_double(this.aV.maxY), MathHelper.floor_double(this.aV.maxZ))
						? this.aH.getLightBrightness(var2, var5, var6)
						: 0.0F;
	}
	...
	public void writeToNBT(NBTTagCompound var1) {
		var1.setTag("Pos", this
				.newDoubleNBTList(new double[] { this.prevPosZ, this.posX + (double) this.be - (double) this.bn, this.posY }));
	...
	public void updateRidden() {
		if (this.riddenByEntity.field_9293_aM) {
			...
			if (this.riddenByEntity != null) {
				...
				for (this.entityRiderPitchDelta += (double) (this.riddenByEntity.rotationYaw
						- this.riddenByEntity.prevRotationYaw); this.entityRiderYawDelta >= 180.0D; this.entityRiderYawDelta -= 360.0D) {
				}
	...
	public void mountEntity(Entity var1) {
		...
		if (var1 == null) {
			if (this.riddenByEntity != null) {
				this.setLocationAndAngles(this.riddenByEntity.prevPosZ,
						this.riddenByEntity.aV.minY + (double) this.riddenByEntity.width, this.riddenByEntity.posY, this.aR,
						this.rotationYaw);
			...
			this.riddenByEntity.aF = null;
			this.riddenByEntity = null;
			this.setLocationAndAngles(var1.prevPosZ, var1.aV.minY + (double) var1.width, var1.posY, this.aR,
					this.rotationYaw);
		} else {
			if (this.riddenByEntity != null) {
				this.riddenByEntity.aF = null;
