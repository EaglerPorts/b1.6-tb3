package net.minecraft.src;

public class EntityItem extends Entity {
	public ItemStack item;
	private int field_803_e;
	public int age = 0;
	public int delayBeforeCanPickup;
	private int health = 5;
	public float field_804_d = (float)(Math.random() * Math.PI * 2.0D);

	public EntityItem(World var1, double var2, double var4, double var6, ItemStack var8) {
		super(var1);
		this.setSize(0.25F, 0.25F);
		this.be = this.width / 2.0F;
		this.setPosition(var2, var4, var6);
		this.item = var8;
		this.aR = (float)(Math.random() * 360.0D);
		this.posZ = (double)((float)(Math.random() * (double)0.2F - (double)0.1F));
		this.motionX = (double)0.2F;
		this.motionY = (double)((float)(Math.random() * (double)0.2F - (double)0.1F));
	}

	protected boolean canTriggerWalking() {
		return false;
	}

	public EntityItem(World var1) {
		super(var1);
		this.setSize(0.25F, 0.25F);
		this.be = this.width / 2.0F;
	}

	protected void entityInit() {
	}

	public void onUpdate() {
		super.onUpdate();
		if(this.delayBeforeCanPickup > 0) {
			--this.delayBeforeCanPickup;
		}

		this.aI = this.prevPosZ;
		this.prevPosX = this.posX;
		this.prevPosY = this.posY;
		this.motionX -= (double)0.04F;
		if(this.aH.getBlockMaterial(MathHelper.floor_double(this.prevPosZ), MathHelper.floor_double(this.posX), MathHelper.floor_double(this.posY)) == Material.lava) {
			this.motionX = (double)0.2F;
			this.posZ = (double)((this.br.nextFloat() - this.br.nextFloat()) * 0.2F);
			this.motionY = (double)((this.br.nextFloat() - this.br.nextFloat()) * 0.2F);
			this.aH.playSoundAtEntity(this, "random.fizz", 0.4F, 2.0F + this.br.nextFloat() * 0.4F);
		}

		this.func_28014_c(this.prevPosZ, this.posX, this.posY);
		this.moveEntity(this.posZ, this.motionX, this.motionY);
		float var1 = 0.98F;
		if(this.aW) {
			var1 = 0.1F * 0.1F * 58.8F;
			int var2 = this.aH.getBlockId(MathHelper.floor_double(this.prevPosZ), MathHelper.floor_double(this.aV.minY) - 1, MathHelper.floor_double(this.posY));
			if(var2 > 0) {
				var1 = Block.blocksList[var2].slipperiness * 0.98F;
			}
		}

		this.posZ *= (double)var1;
		this.motionX *= (double)0.98F;
		this.motionY *= (double)var1;
		if(this.aW) {
			this.motionX *= -0.5D;
		}

		++this.field_803_e;
		++this.age;
		if(this.age >= 6000) {
			this.setEntityDead();
		}

	}

	public boolean handleWaterMovement() {
		return this.aH.handleMaterialAcceleration(this.aV, Material.water, this);
	}

	protected void dealFireDamage(int var1) {
		this.attackEntityFrom((Entity)null, var1);
	}

	public boolean attackEntityFrom(Entity var1, int var2) {
		this.setBeenAttacked();
		this.health -= var2;
		if(this.health <= 0) {
			this.setEntityDead();
		}

		return false;
	}

	public void writeEntityToNBT(NBTTagCompound var1) {
		var1.setShort("Health", (short)((byte)this.health));
		var1.setShort("Age", (short)this.age);
		var1.setCompoundTag("Item", this.item.writeToNBT(new NBTTagCompound()));
	}

	public void readEntityFromNBT(NBTTagCompound var1) {
		this.health = var1.getShort("Health") & 255;
		this.age = var1.getShort("Age");
		NBTTagCompound var2 = var1.getCompoundTag("Item");
		this.item = new ItemStack(var2);
	}

	public void onCollideWithPlayer(EntityPlayer var1) {
		if(!this.aH.multiplayerWorld) {
			int var2 = this.item.stackSize;
			if(this.delayBeforeCanPickup == 0 && var1.inventory.addItemStackToInventory(this.item)) {
				if(this.item.itemID == Block.wood.blockID) {
					var1.triggerAchievement(AchievementList.mineWood);
				}

				if(this.item.itemID == Item.leather.shiftedIndex) {
					var1.triggerAchievement(AchievementList.killCow);
				}

				this.aH.playSoundAtEntity(this, "random.pop", 0.2F, ((this.br.nextFloat() - this.br.nextFloat()) * 0.7F + 1.0F) * 2.0F);
				var1.onItemPickup(this, var2);
				this.setEntityDead();
			}

		}
	}
}
