...
public class EntitySquid extends EntityWaterMob {
	...
	public EntitySquid(World var1) {
		super(var1);
		this.N = "/mob/squid.png";
		this.setSize(0.95F, 0.95F);
		this.field_21080_l = 1.0F / (this.br.nextFloat() + 1.0F) * 0.2F;
	}
	...
	public boolean isInWater() {
		return this.aH.handleMaterialAcceleration(this.aV.expand(0.0D, (double)-0.6F, 0.0D), Material.water, this);
	}
	...
	public void onLivingUpdate() {
		...
		if(this.isInWater()) {
			...
			if(!this.U) {
				this.posZ = (double)(this.randomMotionVecX * this.randomMotionSpeed);
				this.motionX = (double)(this.randomMotionVecY * this.randomMotionSpeed);
				this.motionY = (double)(this.randomMotionVecZ * this.randomMotionSpeed);
			}

			var1 = MathHelper.sqrt_double(this.posZ * this.posZ + this.motionY * this.motionY);
			this.field_9363_r += (-((float)Math.atan2(this.posZ, this.motionY)) * 180.0F / (float)Math.PI - this.field_9363_r) * 0.1F;
			this.aR = this.field_9363_r;
			this.field_21087_c += (float)Math.PI * this.field_21079_m * 1.5F;
			this.field_21089_a += (-((float)Math.atan2((double)var1, this.motionX)) * 180.0F / (float)Math.PI - this.field_21089_a) * 0.1F;
		} else {
	...
	protected void updatePlayerActionState() {
		if(this.br.nextInt(50) == 0 || !this.bw || this.randomMotionVecX == 0.0F && this.randomMotionVecY == 0.0F && this.randomMotionVecZ == 0.0F) {
			float var1 = this.br.nextFloat() * (float)Math.PI * 2.0F;
			this.randomMotionVecX = MathHelper.cos(var1) * 0.2F;
			this.randomMotionVecY = -0.1F + this.br.nextFloat() * 0.2F;
